...
function loadProfile() {
  ...
  if (browser.isExisting('.dialog-404')) {
    return spinnerCrawl.fail(
      chalk.red(`Profile ${this.profileName} doesn't exist!`)
    );
  }

  spinnerCrawl.succeed(
    chalk.green(`Profile successfully loaded for ${this.profileName}!`)
  );
  ...
  this.dataProfile = {
    ...
    numberPosts: utils.cleanNumber(
      getValue('ul._h9luf li:first-child ._fd86t')
    ),
    numberFollowers: utils.cleanNumber(
      getValue('ul._h9luf li:nth-child(2) ._fd86t')
    ),
    numberFollowing: utils.cleanNumber(
...
function extractUrlPostProfile() {
  ...
  function moveToObject() {
    ...
    let numberLikes = getValue(
      `${item} ._mli86 ul li:nth-child(1) span:first-child`
    );
    while (!numberLikes) {
      browser.pause(100);
      numberLikes = getValue(
        `${item} ._mli86 ul li:nth-child(1) span:first-child`
      );
    }
    ...
    const post = {
      ...
      numberComments: utils.cleanNumber(
        getValue(`${item} ._mli86 ul li:nth-child(2) span:first-child`)
      ),
      isVideo: browser.isVisible(`${item} span.coreSpriteVideoIconLarge`),
      multipleImage: browser.isVisible(
    ...
    spinnerCrawl.text = `Advancement of the first step : ${
      self.dataProfile.posts.length
    }/${self.dataProfile.numberPosts}`;
...
function browsePosts() {
  ...
  while (this.urls.length > 0) {
    ...
    const numberLikes = utils.cleanNumber(
      getValue('span._nzn1h span:first-child')
    );
